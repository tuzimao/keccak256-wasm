...
// 克隆Uint32Array
function cloneArrayGeneric<T>(array: StaticArray<T>): StaticArray<T> {
...
class Keccak {
  ...
  hex(): string {
    ...
    while (j < outputBlocks) {
      ...
      if (j % blockCount === 0) {
        s = cloneArrayGeneric<Uint32Array>(s);
        this.permute(s);
        j = 0;
      }
    }
...
export function updateKeccakWithString(ptr: usize, length: i32): void {
  const message = String.UTF8.decodeUnsafe(ptr, length, true);
  keccakInstance.updateString(message);
}

export function updateKeccakWithUint8Array(ptr: usize, length: i32): void {
  const message = new Uint8Array(memory.buffer, ptr, length);
  keccakInstance.updateUint8Array(message);
}
...
export function keccakToHex(): usize {
  if (keccakInstance !== null) {
    const hex = keccakInstance.hex();
    return String.UTF8.encode(hex, true);
  }
  return 0;
}
